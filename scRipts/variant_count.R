...
library(ggplot2)
count_plot <- ggplot() + geom_bar(aes(x = vcf1$CHROM, fill = SV_Type), data=vcf1, position = "stack", stat="count") +
  theme_minimal() + 
  labs(title = "Counts of variants per chromosome", y = "Count", x = "Chromosomes") + 
